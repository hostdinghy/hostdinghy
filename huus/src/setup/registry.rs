...
pub async fn setup(registry: Registry) -> Result<(), CliError> {
	...
	.await
	.with_message("Failed to write $HUUS_DIR/registry/compose.yml")?;

	let config_yml = registry_dir.join("config.yml");
	fs::write(config_yml, CONFIG_YML)
		.await
		.with_message("Failed to write $HUUS_DIR/registry/config.yml")?;

	let password_file = registry_dir.join("registry.password");
	fs::write(password_file, "")
		.await
		.with_message("Failed to write $HUUS_DIR/registry/registry.password")?;
